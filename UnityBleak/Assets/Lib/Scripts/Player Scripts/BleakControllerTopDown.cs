...
public class BleakControllerTopDown : MonoBehaviour {
	...
	void Update () {
		...
		case STATE_HURT:
			...
			switch (numLives){
			case 1:
				skelAnim.skeleton.SetSkin("BleakDamaged01");
				break;
			case 0:
				skelAnim.skeleton.SetSkin("BleakDamaged02");
				break;
			default:
		...
		case STATE_DEAD:
			if (Input.GetKeyDown(KeyCode.R)){
				transform.position = startPoint.position;
				numLives = 2;
				skelAnim.skeleton.SetSkin("Bleak");
				SetState(STATE_NORMAL);
			}
			break;
		}
		...
		if (jumping>=0){
			...
			if (idleDelay >= 7)
				...
				if (Input.GetAxisRaw("Horizontal") == 0 && !Input.GetKey(KeyCode.Space) && idleDelay <=6){
					...
					} else if (skelAnim.state.ToString()=="walkingLeft"){
						skelAnim.state.ClearTrack(0);
						if (numLives > 0) skelAnim.state.AddAnimation(0,"idleDown",true,0.0f);
						else skelAnim.state.AddAnimation(0,"idleUp",true,0.0f);
					} else {
						if (numLives > 0) skelAnim.state.AddAnimation(0,"idleDown",true,0.0f);
						else skelAnim.state.AddAnimation(0,"idleUp",true,0.0f);
					}
	...
	void UpdateForceMove(float dt){
		if ((transform.position - forceMoveDestination.position).sqrMagnitude >= .25f){
			...
			Messenger.Broadcast<Transform,BleakControllerTopDown>("CompletedForceMove",forceMoveDestination,this);
		}
	}
	...
	void UpdateLeftRight(float dt){
		...
		if (directionInt != 0){
			...
			if (runDelay < timeUntilRun){
				...
				if (jumping != -1){
					//play jogging anim
					if (skelAnim.state.ToString()!="walkingLeft" && skelAnim.state.ToString()!="idleDown"){
						if (numLives > 0) skelAnim.state.SetAnimation(0,"walkingLeft",true);
						else skelAnim.state.SetAnimation(0,"idleDown",true);
					}
				}
				runDelay += dt;
			} else {
	...
	void UpdateInputNormal(float dt){
		if (Input.GetKeyDown(actionButton)){
			...
			Messenger.Broadcast<Transform,BleakControllerTopDown>("BleakInteractionActivate",transform,this);
		}
	}
